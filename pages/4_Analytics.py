...
with tab_technician:
    ...
    if not technicians:
        ...
        selected_tech = st.selectbox("Select Technician", list(tech_options.keys()))
        ...
        t_jobs = db.get_jobs_for_technician_in_period(tech_options[selected_tech], t_start, t_end)
        t_df = jobs_to_df(t_jobs)

        if t_df.empty:
            st.info(f"No jobs for {selected_tech} in this period.")
        else:
